...
|Broken breadcrumb links|Broken Breadcrumb link <br/>Breadcrumb: {Breadcrumb text}<br/>URL: {URL}|1. Open the link of article, check the link of the breadcrumb that report error on CATS. <br/> 2. If the breadcrumb link is broken link, this issue should be fixed before release.|
|Broken internal links|	Broken link (internal to Docs)<br/>Link: [Link Text],<br/>URL: {Link URL}|1.Open the link of article and navigate to broken link (could search by {Link Text})<br/>2.Click the link, if it is broken link, this issue should be fixed before release.|
|Broken bookmarks|	Bookmark not working. <br/>Link: [Bookmark Text],<br/>URL: {Bookmark URL}	|1.	Open the link of article, navigate to broken bookmark (could search by {Link Text})<br/>2.	Click the bookmark, if it does not navigate to target anchor, this issue should be fixed before release.|
...
|Breadcrumb links to pages external to Docs	|The breadcrumb [{Breadcrumb text}] is linked to a page [{Page URL}] external to Docs.com.<br/>All breadcrumb links should link to Docs.com pages.|	 All breadcrumb links should link to Docs.com pages, if there is breadcrumb does not link to docs page, this issue should be fixed before release|
|Missing code samples	|Code sample is missing content.	|1.	Open the page URL, open Developer Tools (via press keyboard F12). <br/>2.	On “Elements” tab, search with “<code” and find the code which content is empty.<br/>3.	This issue should be fixed before release|
...
|Missing videos	|A video is missing.<br/>URL: {source URL of the video}	|1.	Open the page URL, open Developer Tools (via press keyboard F12). <br/>2.	On “Elements” tab, search with the video name (could get video name from source URL) to locate to video source.<br/>3.	Right click the source of the video, select “Scroll into view” and navigate to the video.<br/>4. If it does not display on page, check source URL of the video is valid. <br/>5. If source URL is invalid, this issue should be fixed before release. |
|Unplayable videos	|A video is not able to play.<br/>URL: {Video URL}|1.	 Open the page URL, open Developer Tools (via press keyboard F12). <br/>2. On “Elements” tab, search with the video name (could get video name from source URL) to locate to video source.<br/>3. Right click the source of the video, select “Scroll into view” and navigate to the video.<br/>4. Click the “Play” button of the video, if the video cannot play. <br/>5.	This issue should be fixed before release.|
|Missing images	|An image is missing.<br/>Image alt: {image alt}<br/>Image source: {image source url}	|1.	Open the page URL, open Developer Tools (via press keyboard F12). <br/>2.	On “Elements” tab, search with the image name (could get image name from source URL) to locate to image source.<br/>3.	Right click the source of the image, select “Scroll into view” and navigate to the image.<br/>4.	If the image does not display on page, check source URL of the image is valid. <br/>5.	If source URL is invalid, this issue should be fixed before release.|
|Empty list bullets	|A List is missing content.<br/>List: {List context}|1.	Open the page URL, search with {List context}, and navigate to the list. <br/>2.	If there is empty list bullet on list, should fix it.|
|Missing required metadata	|The topic has ms.prod but is missing ms.technology.	|Excepted: The "ms.service" or  "ms.prod + ms.technology" exist in page source<br/>“ms.technology” is required for some ms.prod values. Check the list of approved values to see if you can use the product without a technology. If it does not, this tag is mandatory.|
...
||Missing Metadata: ms.date |Author-populated metadata fields:<br/>Here is a list of mandatory metadata which should be populated by writers:<ul><li>title</li><li>ms.date</li><li>author</li><li>ms.author</li><li>ms.topic</li></ul><br/>More info please refer to metadata spec|
||Missing Metadata: ms.documentid	|OPS-populated metadata fields:<br/>Here is a list of metadata fields populated by the system in the final HTML output from OPS. Authors should not include any of these in the content Markdown files.<ul><li>ms.documentid</li><li>ms.publishtime</li><li>ms.giturl</li><li>ms.sourcegitcommit</li><li>ms.locale</li><li>ms.publishtime</li></ul><br/>More info please refer to metadata spec|
